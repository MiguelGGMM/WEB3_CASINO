...
const Roulette = ({_props}) => {
    ...
    useEffect(() => {
        if(bet && bet.prizeWon != undefined && bet.customPrizeDollarAmountWonType != undefined && bet.NFTwonType != undefined && bet.state == betState.solved) {
            ...
            for(const option of ROULETTE_OPTIONS) {
                if(option.prizeType == _prizeType && option.prizeSubtype == prizeSubtype) {
                    let newPrizeNumber = n;
    ...
    <>
        <Wheel
            ...
            onStopSpinning={() => {
                ...
                onWin(optionSelected.option, optionSelected.image.uri, optionSelected.style.backgroundColor);
            }}
            spinDuration={1.5}
