...
const DashboardSidebar = (/* {} */) => {
  ...
  const [tokenPrice, setTokenPrice] = React.useState(
    store.getState().prices.eth ?? props.eth,
  )
  const loadTokenPrice = () => {
    CommonService.getETHPrice().then((_tokenPrice) => {
      ...
      store.dispatch({
        type: SET_PRICE,
        price: { token: 'eth', price: _tokenPrice },
      })
          ...
          <li className={'nav-item tw-pt-4 tw-relative'}>
            ...
            >
              <div className="icon icon-shape icon-sm shadow border-radius-md bg-white-2 text-center me-2 d-flex align-items-center justify-content-center tw-w-8 tw-h-8">
                <Image
                  priority={true}
                  src={siderbarRoulette}
                  className="navbar-brand-img h-100"
                  alt="rouletteLogo"
                />
              </div>
              <span className="nav-link-text ms-1">Wheel of Fortune</span>
            </a>
          </li>
          <li className="nav-item disabled tw-relative">
            ...
            >
              <div className="icon icon-shape icon-sm shadow border-radius-md bg-white-2 text-center me-2 d-flex align-items-center justify-content-center tw-w-8 tw-h-8">
                <Image
                  priority={true}
                  src={siderbarBlackjack}
                  className="navbar-brand-img h-100"
                  alt="blackjackLogo"
                />
              </div>
              <span className="nav-link-text ms-1 tw-relative">Blackjack</span>
            </a>
          </li>
        </ul>
      </div>
              ...
              {tokenPrice > 0 && (
                <div className="tw-flex-row tw-flex tw-pt-2 align-items-center tw-pl-1 p-3">
                  <div className="col-2 tw-pr-1">
                    <Image
                      priority={true}
                      className="h-100 w-100 tw-pr-1 tw-align-text-top"
                      src={ethLogo}
                      alt="ethLogo"
                    />
                  </div>
                  <div className="tw-text-sm col-2">${tokenPrice}</div>
                </div>
              )}
            </div>
          </div>
