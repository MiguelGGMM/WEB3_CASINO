...
import {
  Roulette__factory,
  SpinsManager__factory,
  ProfitsManager__factory,
  BetsManager__factory,
} from '../abis'
...
const debugBetPending =
  debug.enabled && debug.config.pendingBet.enabled
    ? debug.config.pendingBet
    : undefined
const debugBetPendingClaim =
  debug.enabled && debug.config.pendingBetClaim.enabled
    ? debug.config.pendingBetClaim
    : undefined
const betFromDebug = (
  debugBet: any,
...
export const RouletteService: IRouletteService = {
  ...
  getUserDailyPaidSpins: async (provider) => {
    ...
    const instance = SpinsManager__factory.connect(
      RouletteService.addressSM,
      provider,
    )
  ...
  profitsMadeToday: async (provider) => {
    return (await RouletteService.profitsMaxToday(provider))
      .sub(await RouletteService.amountLeftForDailyMaxProfit(provider))
      .toString()
  },
  profitsMadeWeek: async (provider) => {
    return (await RouletteService.profitsMaxWeek(provider))
      .sub(await RouletteService.amountLeftForWeeklyMaxProfit(provider))
      .toString()
  },
  amountLeftForDailyMaxProfit: async (provider) => {
    ...
    const instance = ProfitsManager__factory.connect(
      RouletteService.addressPM,
      provider,
    )
  ...
  pendingBetsAllPaid: async (provider) => {
    ...
    ////////
    const instance = BetsManager__factory.connect(
      RouletteService.addressBM,
      provider,
    )
  ...
  getPrizeFromBetIndex: async (provider, betIndex) => {
    ...
    if (_bet) {
      ...
      let result = `${_prizeType == prizeType.none ? losePrefix : winPrefix} ${
        prizeTypeText[_prizeType]
      }`
      if (
        _prizeType == prizeType.customPrizeDollarAmount &&
        _bet.customPrizeDollarAmountWonType > 0
      ) {
        const prizeSubtype = _bet.customPrizeDollarAmountWonType
        const amountDollars = await instance.customDollarPrizes(prizeSubtype)
        result = `${result} ${amountDollars}$`
      }
      if (_prizeType == prizeType.NFT && _bet.NFTwonType > 0) {
        const prizeSubtype = _bet.NFTwonType
        const nftAddress = await instance.customNFTPrizes(prizeSubtype)
  ...
  // SIGNATURES
  // Perform paid bet
  performPaidBet: (amount, provider) => {
    const signer = provider.getSigner()
