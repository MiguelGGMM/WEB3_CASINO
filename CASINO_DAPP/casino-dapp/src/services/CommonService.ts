...
const fromWei = (value: ethers.BigNumberish) => { return ethers.utils.formatEther(value); }
...
const CommonService: ICommonService = {
    ...
    getETHValue: async (ethAmount) => {
        let ethPrice = await CommonService.getETHPrice();
    ...
    getETHPrice: async () => {
        try{
            if(CommonService.ethPriceCache == 0){
                let data = await axios.get('https://api.coinbase.com/v2/prices/ETH-USD/spot');
