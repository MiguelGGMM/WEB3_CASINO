...
export const CasinoTreasuryService: ICasinoTreasuryService = {
    ...
    deposit: async (provider, tokensQuantity) => {
        const signer = provider.getSigner();
        if(!CasinoTreasuryService.address || CasinoTreasuryService.address == undefined) {
            CommonService.throwErrorMissingVar("NEXT_PUBLIC_CASINOADR");
        } else {
            const contract = CasinoTreasury__factory.connect(CasinoTreasuryService.address, signer);
            return contract.depositTokens({ value: tokensQuantity });
        }
    },
    withdraw: async (provider, tokensQuantity) => {
        const signer = provider.getSigner();
        if(!CasinoTreasuryService.address || CasinoTreasuryService.address == undefined) {
            CommonService.throwErrorMissingVar("NEXT_PUBLIC_CASINOADR");
        } else {
    ...
    getTokensValueDollars: async (provider) => {
        ...
        let _value = await CommonService.getETHValue(tokensAmount.toString());
        return _value;
    },    
    // User casino wallet
    getTokensAmountIWallet: async (provider) => {
        ...
        if(!CasinoTreasuryService.address || CasinoTreasuryService.address == undefined) {
            ...
            const instance = CasinoTreasury__factory.connect(CasinoTreasuryService.address, provider);
            const balance = await instance.balanceOf(owner);
            return balance;
        }
    },
    getTokensValueDollarsIWallet: async (provider) => {
        const tokensAmount = await CasinoTreasuryService.getTokensAmountIWallet(provider);
        if(!CasinoTreasuryService.address || CasinoTreasuryService.address == undefined) {
            CommonService.throwErrorMissingVar("NEXT_PUBLIC_CASINOADR");
        } else {
            const instance = CasinoTreasury__factory.connect(CasinoTreasuryService.address, provider);
            return await instance.calcDollars(tokensAmount);
        }
    },
