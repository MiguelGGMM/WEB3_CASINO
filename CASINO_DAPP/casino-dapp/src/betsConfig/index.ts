...
const getOptions = (): types.optionType[] => {
  return (Object.keys(prizeType) as types.prizeTypeKeys[])
    .map((k) => {
      if (
        prizeType[k] != prizeType.customPrizeDollarAmount &&
        prizeType[k] != prizeType.NFT
      ) {
    ...
    .concat(
      (
        Object.keys(
          contractConfig.customDollarPrizeSubtype,
        ) as types.customDollarPrizeSubtypeKeys[]
      ).map((k) => {
        const oc =
          rouletteConfig.optionsCustom[
